                                    ...
                                    FxU32 dst_stride, void *dst_data ))
{
  ...
  if ( grLfbLock( GR_LFB_READ_ONLY,
    ...
    for (scanline = 0; scanline < src_height; scanline++) {
      FxU16 *dstLine = (FxU16 *)(((char *)dst_data) + scanline * dst_stride);
      FxU32 x;

      if (scanline == 0 || scanline == 1 || scanline == 15 ||
          scanline == 63 || scanline == 127 || scanline == 255 ||
          scanline + 1 == src_height) {
        snprintf(stage, sizeof(stage), "grLfbReadRegion: scanline %u", (unsigned)scanline);
      }

      for (x = 0; x < src_width; x += 2) {
        volatile void *srcAddr = ((char *)info.lfbPtr) +
                                 ((src_y + scanline) * info.strideInBytes) +
                                 ((src_x + x) << 1);
#ifdef SIM_BACKEND
        ...
        dstLine[x] = (FxU16)(pair & 0xFFFF);
        if ((x + 1) < src_width) {
          dstLine[x + 1] = (FxU16)(pair >> 16);
        }
      }
    }
