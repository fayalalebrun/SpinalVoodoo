...
#ifdef DE10_BACKEND
static void de10EnumStage(const char *stage) {
...
#else
static void de10EnumStage(const char *stage) { (void)stage; }
#endif
...
initEnumHardware( InitHWEnumCallback *cb )
{
  ...
  FxU32  device;
  if ( !libInitialized ) {
    de10EnumStage("initEnumHardware: begin");
    ...
    for( busLocation = 0; busLocation < MAX_PCI_DEVICES; busLocation++ ) {
      if ( pciDeviceExists( busLocation ) ) {
        ...
        if ( (vId == TDFXVID) &&
          ...
          de10EnumStage("initEnumHardware: after initMapBoard");
#endif
          sst1InitGetDeviceInfo( base, &info );
          de10EnumStage("initEnumHardware: after sst1InitGetDeviceInfo");
...
initMapBoard(const FxU32 boardNum)
{
   ...
   if (okP) {
      ...
      if (okP) {
         de10EnumStage("initMapBoard: before sst1InitMapBoard");
         retVal = sst1InitMapBoard(boardNum);
         de10EnumStage("initMapBoard: before sst1InitRegisters");
         sst1InitRegisters((FxU32*)retVal);
         de10EnumStage("initMapBoard: after sst1InitRegisters");
      }
#elif defined(SST96)
      okP = (IS_CHIP(AT3D) ||
             IS_CHIP(MCRX));
