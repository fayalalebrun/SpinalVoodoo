...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] getTmuConfigData: begin\n");
    fflush(stderr);
#endif
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] getTmuConfigData: raw tmuConfig=0x%x\n", info->tmuConfig);
    fflush(stderr);
#endif
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] getTmuConfigData: done tmuConfig=0x%x\n", info->tmuConfig);
    fflush(stderr);
#endif
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] sense: tmu=%u mem=0x%x init=0x%x begin\n", tmu, mem, init);
    fflush(stderr);
#endif
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] sense: tmu=%u mem=0x%x readback=0x%08x\n", tmu, mem, mem);
    fflush(stderr);
#endif
	...
	FxU32 *TmuMemorySize)
{
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] sst1InitGetTmuMemory: tmu=%u begin\n", tmu);
    fflush(stderr);
#endif
	...
	*TmuMemorySize = 4;
	#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	fprintf(stderr, "[de10] sst1InitGetTmuMemory: tmu=%u size=4\n", tmu);
	fflush(stderr);
	#endif
	return(FXTRUE);
    }
	...
	*TmuMemorySize = 2;
	#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	fprintf(stderr, "[de10] sst1InitGetTmuMemory: tmu=%u size=2\n", tmu);
	fflush(stderr);
	#endif
	return(FXTRUE);
    }
	...
	*TmuMemorySize = 1;
	#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	fprintf(stderr, "[de10] sst1InitGetTmuMemory: tmu=%u size=1\n", tmu);
	fflush(stderr);
	#endif
	return(FXTRUE);
    }
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] sst1InitGetTmuInfo: begin\n");
    fflush(stderr);
#endif
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] sst1InitGetTmuInfo: after initSumTables\n");
    fflush(stderr);
#endif
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] sst1InitGetTmuInfo: after getTmuConfigData tmuConfig=0x%x\n", info->tmuConfig);
    fflush(stderr);
#endif
...
static int fbiMemSize(FxU32 *sstbase)
{
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	fprintf(stderr, "[de10] fbiMemSize: begin\n");
	fflush(stderr);
#endif
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	fprintf(stderr, "[de10] fbiMemSize: probing 4MB path\n");
	fflush(stderr);
#endif
	...
	sst1InitIdleFBI(sstbase);
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	fprintf(stderr, "[de10] fbiMemSize: reading 4MB probe pixels\n");
	fflush(stderr);
#endif
	{
		FxU32 probeA;
		FxU32 probeB;
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
		fprintf(stderr, "[de10] fbiMemSize: reading probeA (128,100)\n");
		fflush(stderr);
#endif
		probeA = LFB_GETPIXEL(128, 100);
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
		fprintf(stderr, "[de10] fbiMemSize: probeA=0x%04x\n", probeA & 0xffff);
		fprintf(stderr, "[de10] fbiMemSize: reading probeB (200,200)\n");
		fflush(stderr);
#endif
		probeB = LFB_GETPIXEL(200, 200);
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
		fprintf(stderr, "[de10] fbiMemSize: probeB=0x%04x\n", probeB & 0xffff);
		fflush(stderr);
#endif
		if((probeA == 0xdead) &&
		   (probeB == 0x55aa)) {
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	fprintf(stderr, "[de10] fbiMemSize: probing 2MB path\n");
	fflush(stderr);
#endif
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	fprintf(stderr, "[de10] fbiMemSize: reading 2MB probe pixels\n");
	fflush(stderr);
#endif
...
check1MByte:
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	fprintf(stderr, "[de10] fbiMemSize: probing 1MB path\n");
	fflush(stderr);
#endif
	ISET(sst->lfbMode, SST_LFB_565 | SST_LFB_WRITEFRONTBUFFER |
		SST_LFB_READFRONTBUFFER);
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	fprintf(stderr, "[de10] fbiMemSize: reading 1MB probe pixels\n");
	fflush(stderr);
#endif
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	fprintf(stderr, "[de10] fbiMemSize: done retval=%d\n", retval);
	fflush(stderr);
#endif
...
FxBool sst1InitFillDeviceInfo(FxU32 *sstbase, sst1DeviceInfoStruct *info)
{
	...
	for (i=0; i<5; i++) {
	  #if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	  fprintf(stderr, "[de10] sst1InitFillDeviceInfo: attempt=%d begin\n", i);
	  fflush(stderr);
	  #endif
	  if (i)
	  ...
	  if(sst1InitGetFbiInfo(sstbase, info) == FXFALSE)
	    continue;
	  #if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	  fprintf(stderr, "[de10] sst1InitFillDeviceInfo: attempt=%d after GetFbiInfo\n", i);
	  fflush(stderr);
	  #endif
	  ...
	  if(sst1InitGetTmuInfo(sstbase, info) == FXFALSE)
	    continue;
	  #if defined(DE10_BACKEND) || defined(SIM_BACKEND)
	  fprintf(stderr, "[de10] sst1InitFillDeviceInfo: attempt=%d after GetTmuInfo\n", i);
	  fflush(stderr);
	  #endif
	  break;
	}
