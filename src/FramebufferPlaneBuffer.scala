...
case class FramebufferPlaneBuffer(c: Config, formalStrong: Boolean = true) extends Component {
  ...
  val activePushReady = Mux(activeSlot === 0, slot0Push.ready, slot1Push.ready)
  ...
  if (c.useFbWriteBuffer) {
    when(drainState === DrainState.Send) {
      ...
      when(drainPending) {
        ...
        when(drainSlot === 0) {
          drainData := slot0Pop.payload.data
          drainMask := slot0Pop.payload.mask
        }.otherwise {
          drainData := slot1Pop.payload.data
          drainMask := slot1Pop.payload.mask
        }
      }

      io.mem.cmd.valid := drainPending || Mux(drainSlot === 0, slot0Pop.valid, slot1Pop.valid)
      ...
      when(writeReqPipe.fire) {
        when(!activeValid) {
          ...
          when(activeSlot === 0) {
            ...
            slot0Push.payload.address := activePendingAddr
            slot0Push.payload.data := activePendingData
            slot0Push.payload.mask := activePendingMask
          }.otherwise {
            slot1Push.valid := True
            slot1Push.payload.address := activePendingAddr
            slot1Push.payload.data := activePendingData
            slot1Push.payload.mask := activePendingMask
          }
    ...
    when(directState === DirectState.Idle && writeReqPipe.valid) {
      writeReqPipe.ready := True
      directWriteAddress := writeReqPipe.payload.address
      directWriteData := writeReqPipe.payload.data
      directWriteMask := writeReqPipe.payload.mask
      directState := DirectState.Cmd
    }

    when(directState === DirectState.Cmd) {
      io.mem.cmd.valid := True
      io.mem.cmd.fragment.address := directWriteAddress
      io.mem.cmd.fragment.opcode := Bmb.Cmd.Opcode.WRITE
      io.mem.cmd.fragment.length := 3
      io.mem.cmd.fragment.source := 0
      io.mem.cmd.fragment.data := directWriteData
      io.mem.cmd.fragment.mask := directWriteMask
      io.mem.cmd.last := True
      when(io.mem.cmd.fire) {
        directState := DirectState.Rsp
      }
    }

    when(directState === DirectState.Rsp) {
      io.mem.rsp.ready := True
      when(io.mem.rsp.fire) {
        directState := DirectState.Idle
      }
    }
  }
