...
case class SimBmbMemory(p: BmbParameter, size: Int, timing: SimMemoryTiming) extends Component {
  ...
  private def readLatency(firstBeat: Boolean): UInt = timing.mode match {
    case "fixed" => latencyValue(timing.fixedReadLatency)
    case "burst" =>
      latencyValue(if (firstBeat) timing.burstSetupLatency else timing.burstBeatLatency)
    case _ => latencyValue(0)
  }
  ...
  when(doReadCmd) {
    ...
    rspDelay := readLatency(firstBeat = true)
  }
  ...
  when(doRsp) {
    when(state === OpState.ReadRsp && wordsRemaining > 1) {
      ...
      rspDelay := readLatency(firstBeat = false)
    }.otherwise {
      state := OpState.Idle
      wordsRemaining := 0
      rspDelay := 0
