...
case class BmbBusInterface(
  ...
  /** Create a command register that produces a Stream
    ...
    * added.
    */
  private def bufferedCommandStream[T <: Data](addr: BigInt, src: Stream[T]): Stream[T] = {
    val s2m = src.s2mPipe()
  ...
  def newCommandReg(
    ...
    (VoodooRegInst(reg), bufferedCommandStream(addr, pulseStream))
  }
  ...
  def newCommandRegWithPayload[T <: Data](
    ...
    (reg, bufferedCommandStream(addr, src))
  }
