...
object ColorCombine {
  ...
  object Config {
    def fromFbzColorPath(fcp: FbzColorPath): Config = {
      val cfg = Config()
  ...
  object Input {
    private def u8FromSat(v: AFix): UInt = (v.asBits >> 12).asUInt.resize(8 bits)
    ...
    def fromTmuAndRaster(
      ...
      out.coords.assignFromVec(rasterOut.coords)
      out.iterated.r := u8FromSat(redSat)
      out.iterated.g := u8FromSat(greenSat)
      out.iterated.b := u8FromSat(blueSat)
      out.iteratedAlpha := (alphaSat.asBits >> 30).asUInt.resize(8 bits)
      out.iteratedZ := u8FromSat(depthSat)
...
case class ColorCombine(c: voodoo.Config) extends Component {
  ...
  val COORDS = Payload(PixelCoords(c))
  ...
  def loadPassThrough(self: NodeBaseApi, payload: Input): Unit = {
  ...
  def selectRgbOther(self: NodeBaseApi, payload: Input): Unit = {
  ...
  def selectRgbLocal(self: NodeBaseApi, payload: Input): Unit = {
  ...
  def selectAlphaOther(self: NodeBaseApi, payload: Input): Unit = {
  ...
  def selectAlphaLocal(self: NodeBaseApi, payload: Input): Unit = {
  ...
  def storePassThrough(payload: Output, self: NodeBaseApi): Unit = {
  ...
  n0.driveFrom(io.input) { (self, payload) =>
    ...
    selectRgbOther(self, payload)
    selectRgbLocal(self, payload)
    selectAlphaOther(self, payload)
    selectAlphaLocal(self, payload)
  }
