...
case class Dither() extends Component {
  ...
  def truncIfDisabled[T <: Data](enabled: Bool, dithered: T, truncated: T): T =
    enabled ? dithered | truncated
  ...
  io.output.payload.ditR := truncIfDisabled(
    rLinked.enable,
    romRResult.payload.value,
    rLinked.truncR
  )
  io.output.payload.ditG := truncIfDisabled(rLinked.enable, ditGRom, rLinked.truncG)
  io.output.payload.ditB := truncIfDisabled(rLinked.enable, ditBRom, rLinked.truncB)
...
object Dither {
  ...
  object Input {
    def fromPreDither(pd: Write.PreDither): Input = {
      val ditIn = Input()
      ditIn.r := pd.r
      ditIn.g := pd.g
      ditIn.b := pd.b
